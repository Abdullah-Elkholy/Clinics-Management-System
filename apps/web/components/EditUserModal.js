    ...
    <ModalWrapper open={open} onClose={onClose}>
      <div className="bg-white rounded-2xl shadow-xl p-6 w-full max-w-md">
        ...
        <div className="flex justify-end gap-3 mt-4">
          <button type="button" onClick={onClose} className="px-4 py-2 rounded bg-gray-100">إلغاء</button>
          <button type="button" disabled={!canSave} onClick={() => { if (!canSave) return; onSave && onSave({ first, last, username }); onClose && onClose() }} className={`px-4 py-2 rounded text-white ${canSave ? 'bg-blue-600' : 'bg-gray-300 cursor-not-allowed'}`}>حفظ</button>
        </div>
      </div>
    </ModalWrapper>
  )
