    ...
    <ModalWrapper open={open} onClose={onClose}>
      <div className="bg-white rounded-2xl p-6 w-full max-w-3xl shadow-xl">
        ...
        <div className="flex justify-end gap-3">
          ...
          <button disabled={!canSend} onClick={()=>{ if (!canSend) return; onSend(text); onClose() }} className={`px-4 py-2 rounded-lg text-white ${canSend ? 'bg-gradient-to-r from-purple-600 to-blue-600' : 'bg-gray-300 cursor-not-allowed'}`}>إرسال</button>
        </div>
      </div>
    </ModalWrapper>
  )
