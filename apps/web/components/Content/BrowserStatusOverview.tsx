...
export default function BrowserStatusOverview() {
  ...
  const loadBrowserStatuses = useCallback(async () => {
    try {
      ...
      setError(err?.message || 'فشل تحميل حالة المتصفحات');
      addToast(err?.message || 'فشل تحميل حالة المتصفحات', 'error');
    } finally {
      setIsLoading(false);
    }
  }, [addToast]);
  ...
  const handleRefreshAll = useCallback(async () => {
    ...
    try {
      ...
      addToast(err?.message || 'فشل تحديث بعض المتصفحات', 'error');
    } finally {
  ...
  if (isLoading) {
    return (
      <PanelWrapper>
        <PanelHeader title="حالة المتصفحات" />
        <div className="flex items-center justify-center min-h-[400px]">
          <div className="text-center">
            <div className="inline-block animate-spin rounded-full h-12 w-12 border-b-2 border-blue-600 mb-4"></div>
            <p className="text-gray-600">جاري تحميل حالة المتصفحات...</p>
          </div>
        </div>
      </PanelWrapper>
    );
            ...
            {filteredStatuses.map((status) => (
              ...
              >
                ...
                <div className="flex items-center justify-between mb-4">
                  <div>
                    <h3 className="text-lg font-semibold text-gray-900">{status.moderatorName}</h3>
