...
// Helper predicates to replace deprecated isDefault/hasCondition booleans
function isDefaultTemplate(t: MessageTemplate): boolean {
  return t.condition?.operator === 'DEFAULT';
}

function hasActiveCondition(t: MessageTemplate): boolean {
  const op = t.condition?.operator;
  return !!op && !['DEFAULT', 'UNCONDITIONED'].includes(op);
}
...
export default function ManageConditionsModal() {
  ...
  const handleToggleCondition = useCallback(async (template: MessageTemplate) => {
    ...
    // If currently active (hasActiveCondition=true), convert to UNCONDITIONED
    if (hasActiveCondition(template)) {
      ...
      try {
        ...
        // Update to UNCONDITIONED operator
        await messageApiClient.updateCondition(conditionBackendId, {
  ...
  const activeConditions = useMemo(
    () => messageConditions.filter(c => {
      ...
      return template && hasActiveCondition(template);
    }),
                  ...
                  {isEditing ? (
                    ...
                    <div className={`p-4 border-2 rounded-lg transition cursor-pointer ${
                      ...
                      <div className="space-y-2">
                        ...
                        {hasActiveCondition(template) && condition && (
                        ...
                        <div className="flex gap-2 pt-2">
                          ...
                          {!isDefaultTemplate(template) && (
                            ...
                            >
                              ...
                              {hasActiveCondition(template) ? 'تعديل' : 'إضافة شرط'}
                            </button>
                          )}
                        </div>
                      </div>
