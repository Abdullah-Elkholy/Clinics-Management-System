...
public class WhatsAppService_monolithic
{
    ...
    public async Task<bool> SendMessageAsync(string phoneNumber, string message)
    {
        ...
        try
        {
            ...
            // Detect WhatsApp Web loading chats ("Loading your chats %")
            var loadingSelector = "span[aria-label^='Loading your chats']";
            var loadingElement = await page.QuerySelectorAsync(loadingSelector);
            if (loadingElement != null)
            {
                var loadingText = await loadingElement.InnerTextAsync();
                Console.WriteLine($"WhatsApp Web is loading chats: {loadingText}. This may take a while if you haven't opened WhatsApp recently.");
                // Wait for the loading element to disappear
                await page.WaitForSelectorAsync(loadingSelector, new PageWaitForSelectorOptions { State = WaitForSelectorState.Detached, Timeout = 0 });
                Console.WriteLine("WhatsApp chats finished loading.");
            }
            ...
            if (input is null)
                throw new InvalidOperationException("Message input box not found.");
            ...
            if (!sent && iconType == "msg-time")
            {
                ...
                if (statusIcon != null)
                {
                    var tempAttribute = await statusIcon.GetAttributeAsync("data-icon");
                    iconType = tempAttribute ?? string.Empty;
